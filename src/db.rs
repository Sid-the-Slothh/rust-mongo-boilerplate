...
impl DB {
    ...
    // Since our implementation does not have any sensitive fields, the method will only map the fields in NoteModel to the fields in NoteResponse
    ...
    // Takes in data from a request payload and converts it into a mongoDB document, i.e creating the new note.
    ...
    // Create a note, make _id the hash of the title to avoid dup titles, then recover the doc with find_one, then return the success with doc.
    ...
    // Use $set to make an updated NoteDoc and then use find one and update to replace it in the DB.
    ...
    // Delete an item by filtering for a doc based on the provided _id
